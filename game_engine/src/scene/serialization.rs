use crate::domain::physics::ShapeType;
use crate::ecs::{
...
impl SerializedScene {
    ...
    /// 从World序列化场景
    pub fn from_world(world: &mut World, name: impl Into<String>) -> Self {
        ...
        for entity_ref in world.query::<EntityRef>().iter(world) {
            let mut serialized_entity = SerializedEntity {
                id: entity_ref.id().to_bits(),
                ..Default::default()
            };

            // 序列化Transform组件
            if let Some(transform) = world.get::<Transform>(entity_ref.id()) {
            ...
            // 序列化Sprite组件
            if let Some(sprite) = world.get::<Sprite>(entity_ref.id()) {
                serialized_entity.components.insert(
                    "Sprite".to_string(),
            ...
            // 序列化PointLight组件
            if let Some(light) = world.get::<PointLight>(entity_ref.id()) {
                serialized_entity.components.insert(
            ...
            // 序列化Camera组件
            if let Some(camera) = world.get::<Camera>(entity_ref.id()) {
            ...
            // 序列化ColliderDesc组件
            if let Some(col) = world.get::<ColliderDesc>(entity_ref.id()) {
            ...
            if let Some(mat) = world.get::<PbrMaterialComp>(entity_ref.id()) {
                serialized_entity.components.insert(
            ...
            // 序列化PointLight3D组件
            if let Some(light) = world.get::<PointLight3D>(entity_ref.id()) {
                serialized_entity.components.insert(
            ...
            if let Some(light) = world.get::<DirectionalLightComp>(entity_ref.id()) {
                serialized_entity.components.insert(
