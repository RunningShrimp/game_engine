...
impl PluginConfigManager {
    ...
    /// 内部加载配置
    fn load_configs_internal(
        path: &PathBuf,
    ) -> Result<HashMap<String, PluginConfig>, Box<dyn std::error::Error>> {
        if path.exists() {
    ...
    pub fn enabled_plugins(&self) -> Vec<&PluginConfig> {
        let mut plugins: Vec<_> = self.configs.values().filter(|c| c.enabled).collect();
