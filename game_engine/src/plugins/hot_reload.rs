...
impl PluginHotReloadManager {
    ...
    pub fn new(plugin_directory: impl Into<PathBuf>) -> Result<Self, HotReloadError> {
        ...
        run_sync(async move {
            tokio::fs::create_dir_all(&plugin_dir_clone)
                .await
    ...
    pub fn load_plugin(
        ...
        let metadata = run_sync(async move {
            tokio::fs::metadata(plugin_path_clone)
                .await
                .map_err(|e| HotReloadError::FileSystemError(e.to_string()))
        })?;
        ...
        unsafe {
            ...
            if plugin_ptr.is_null() {
                return Err(HotReloadError::LoadError(
            ...
            if plugin_ptr2.is_null() {
                ...
                return Err(HotReloadError::LoadError(
                    "Failed to create plugin instance for storage".to_string(),
                ));
            }
    ...
    pub fn scan_and_load(
        ...
        let entries = run_sync(async move {
            ...
            let mut dir = tokio::fs::read_dir(&plugin_dir)
                .await
                .map_err(|e| HotReloadError::FileSystemError(e.to_string()))?;
        ...
        for entry in entries {
            ...
            if path.is_file() {
                let extension =
                    path.extension().and_then(|e: &std::ffi::OsStr| e.to_str()).unwrap_or("");
                ...
                if matches!(extension, "so" | "dylib" | "dll") {
                    if let Err(e) = self.load_plugin(&path, registry) {
                        ...
                        path.file_stem().and_then(|n: &std::ffi::OsStr| n.to_str())
                    {
...
pub extern "C" fn create_plugin() -> *mut dyn EnginePlugin {
    ...
    unsafe { std::mem::transmute::<[usize; 2], *mut dyn EnginePlugin>([0, 0]) }
}
