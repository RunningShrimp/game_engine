...
impl EditorContext {
    pub fn new<T: Window>(window: &T, device: &wgpu::Device, format: wgpu::TextureFormat) -> Self {
    ...
    pub fn handle_event<T: Window>(&mut self, window: &T, event: &WindowEvent) -> bool {
        let response = self.state.on_window_event(window, event);
        response.consumed
    }

    pub fn begin_frame<T: Window>(&mut self, window: &T) {
        let raw_input = self.state.take_egui_input(window);
        self.context.begin_frame(raw_input);
    }

    pub fn end_frame<T: Window>(&mut self, window: &T) -> (Vec<egui::ClippedPrimitive>, &mut Renderer) {
