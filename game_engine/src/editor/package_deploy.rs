...
<dict>
    ...
    <string>{}</string>
        ...
        // 创建DMG镜像
        let dmg_path = output_dir.join(format!(
            "{}-{}-macos.dmg",
            self.config.app_name, self.config.version
        ));
    ...
    /// 打包Android应用
    fn package_android(&self, _build_path: &Path, output_dir: &Path) -> Result<PathBuf, String> {
        // Android打包需要使用Android SDK工具
        // 这里提供简化的实现
        let _apk_path = output_dir.join(format!(
            "{}-{}.apk",
            self.config.app_name, self.config.version
        ));
    ...
    /// 打包iOS应用
    fn package_ios(&self, _build_path: &Path, output_dir: &Path) -> Result<PathBuf, String> {
        // iOS打包需要使用Xcode工具
        let _ipa_path = output_dir.join(format!(
            "{}-{}.ipa",
            self.config.app_name, self.config.version
        ));
    ...
    /// 部署到目标平台
    pub fn deploy(&self, _package_path: &Path, target: DeployTarget) -> Result<String, String> {
        match target {
            DeployTarget::Local => self.deploy_local(_package_path),
            DeployTarget::ItchIo => self.deploy_itch(_package_path),
            DeployTarget::Steam => self.deploy_steam(_package_path),
            DeployTarget::GitHub => self.deploy_github(_package_path),
        }
    }

    /// 部署到本地
    fn deploy_local(&self, _package_path: &Path) -> Result<String, String> {
        Ok("Package ready for local deployment".to_string())
    }

    /// 部署到Itch.io
    fn deploy_itch(&self, _package_path: &Path) -> Result<String, String> {
        // 使用butler工具上传到Itch.io
        Err("Itch.io deployment requires butler tool and configuration".to_string())
    }

    /// 部署到Steam
    fn deploy_steam(&self, _package_path: &Path) -> Result<String, String> {
        // 使用SteamCMD上传到Steam
        Err("Steam deployment requires SteamCMD and Steamworks SDK".to_string())
    }

    /// 部署到GitHub Releases
    fn deploy_github(&self, _package_path: &Path) -> Result<String, String> {
