...
impl GpuCommandGenerator {
    ...
    pub fn with_workgroup_size(device: &Device, max_instances: u32, workgroup_size: u32) -> Self {
        ...
        // 创建管线布局，添加push constant范围用于传递index_count
        ...
        let pipeline = device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
            ...
            module: &shader,
            entry_point: Some("generate_commands"),
...
/// 从可见实例生成间接绘制命令。
/// 每个可见实例生成一个绘制命令。
const COMMAND_GENERATOR_SHADER: &str = r#"struct GpuInstance {
    model: mat4x4<f32>,
    aabb_min: vec3<f32>,
    instance_id: u32,
    aabb_max: vec3<f32>,
...
// 通过push constant接收index_count
@push_constant struct PushConstants {
    index_count: u32,
};

@compute @workgroup_size(64)
fn generate_commands(@builtin(global_invocation_id) global_id: vec3<u32>, @push_constant push: PushConstants) {
    let idx = global_id.x;
    ...
    indirect_commands[idx] = DrawIndexedIndirectArgs {
        index_count: push.index_count,    // 每个实例的索引数（从push constant获取）
