...
//
//  ```ignore
//  use game_engine::render::domain_objects::{RenderObject, RenderStrategy, RenderScene};
//
//  // 创建渲染对象
...
impl RenderObject {
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::{RenderObject, RenderObjectId};
    /// use game_engine::render::mesh::GpuMesh;
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::RenderObject;
    ...
    ///
    /// // 更新可见性
    /// // obj.update_visibility(&frustum).expect("Test: operation should succeed");
    /// ```
    ...
    /// // let lod = obj.select_lod(10.0, &mut lod_selector, 0.016).expect("Test: operation should succeed");
    /// ```
    pub fn select_lod(
        &mut self,
        distance: f32,
    ...
    ///
    /// // 从错误恢复
    /// obj.recover_from_error().expect("Test: operation should succeed");
    ...
    ///
    /// // 更新位置
    /// obj.update_position(Vec3::new(10.0, 5.0, 0.0)).expect("Test: operation should succeed");
    /// ```
    ...
    ///     scale: Vec3::ONE,
    /// };
    /// obj.update_transform(new_transform).expect("Test: operation should succeed");
    /// ```
...
///
/// ```rust
/// use game_engine::render::domain_objects::{RenderObject, RenderObjectCompensation};
///
/// // 保存当前状态
...
impl RenderObjectCompensation {
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::{RenderObjectCompensation, RenderObjectId};
    ///
    /// let compensation = RenderObjectCompensation::new(
    ///     RenderObjectId::new(1),
    ///     true,
...
///
/// ```rust
/// use game_engine::render::domain_objects::{RenderStrategy, RenderObject};
///
/// // 为对象选择策略
...
impl RenderStrategy {
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::RenderStrategy;
    ///
    /// // 实例数量超过阈值，使用实例化
    /// let strategy = RenderStrategy::select_for_instances(15, true);
    /// assert!(matches!(strategy, RenderStrategy::Instanced));
...
impl RenderScene {
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::RenderScene;
    ///
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::{RenderScene, RenderObject, RenderObjectId};
    /// use game_engine::render::mesh::GpuMesh;
    /// use std::sync::Arc;
    ///
    /// let mut scene = RenderScene::new();
    ...
    ///
    /// scene.add_object(obj).expect("Test: operation should succeed");
    ...
    /// scene.update(0.016, camera_pos).expect("Test: operation should succeed");
    /// ```
...
impl RenderCommand {
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::{RenderCommand, RenderStrategy, RenderObjectId};
    ///
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::{RenderCommand, RenderStrategy};
    ///
...
///
/// ```rust
/// use game_engine::render::domain_objects::{PbrScene, LightSource};
/// use glam::Vec3;
///
...
/// scene.add_light(light).expect("Test: operation should succeed");
///
/// // 验证场景
...
impl PbrScene {
    ...
    ///
    /// ```rust
    /// use game_engine::render::domain_objects::PbrScene;
    ///
    ...
    /// scene.add_light(point_light).expect("Test: operation should succeed");
    /// assert_eq!(scene.light_count(), 1);
    ///
    /// // 添加方向光
    ...
    ///     0.9,
    /// ).expect("Test: operation should succeed");
    /// scene.add_light(dir_light).expect("Test: operation should succeed");
...
///
/// ```rust
/// use game_engine::render::domain_objects::LightSource;
/// use glam::Vec3;
///
/// // 创建有效的点光源
...
impl LightSource {
    ...
    /// use glam::Vec3;
    ///
    /// let light = LightSource::new_point_light(Vec3::ZERO, Vec3::ONE, 1.5, 10.0).expect("Test: operation should succeed");
    /// assert_eq!(light.intensity(), 1.5);
    /// ```
...
mod tests {
    ...
    use crate::render::lod::{LodConfig, LodConfigBuilder, LodSelector};
    use bevy_ecs::prelude::*;
...
#[ignore]  // TODO: Fix compilation errors
    fn test_light_source_from_ecs_point_light_valid() {
        ...
        assert!(light.expect("Test: operation should succeed").is_valid());
    }
