...
impl Ord for BatchKey {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        self.pipeline_id
            ...
            .then(self.mesh_id.cmp(&other.mesh_id))
            .then(self.material_id.cmp(&other.material_id))
    }
}
...
impl BatchManager {
    ...
    fn split_batch(&mut self, batch: InstanceBatch) {
        ...
        // 基于空间位置的批次拆分（空间哈希）
        // 1. 计算所有实例的中心点
        let mut instance_centers: Vec<(usize, [f32; 3])> = Vec::with_capacity(instance_count);
        ...
        for (i, instances) in batch_instances.into_iter().enumerate() {
            ...
            let new_key = BatchKey {
                ...
                pipeline_id: batch.key.pipeline_id,
                blend_mode: batch.key.blend_mode,
                depth_test: batch.key.depth_test,
                render_flags: batch.key.render_flags,
            };
    ...
    fn merge_small_batches(&mut self) {
        // 更新small_batch_keys，确保它包含所有符合条件的小批次
        ...
        // 处理每个渲染状态组
        for (render_state_key, batch_keys) in render_state_groups {
            if batch_keys.len() < 2 {
                continue; // 至少需要2个批次才能合并
            }

            // 在渲染状态组内，尝试合并不同材质/网格的小批次
            self.merge_cross_material_batches(render_state_key, batch_keys);
        }
    ...
    fn attempt_cross_material_merge(
        ...
        let new_key = BatchKey {
            ...
            material_id: source_batch.key.material_id,
            pipeline_id: source_batch.key.pipeline_id,
            blend_mode: source_batch.key.blend_mode,
    ...
    /// 合并一组批次
    fn merge_batch_group(&mut self, group_key: BatchKey, batch_keys: Vec<BatchKey>) {
        ...
        // 更新统计信息
        self.stats.batches_merged += (merged_count - 1) as u32;

        // 重新插入合并后的批次
        // 使用group_key作为合并后的批次键，确保渲染状态一致
        self.batches.insert(group_key, base_batch);
