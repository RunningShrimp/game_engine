...
//  ## 架构改进
//
//  将原来的 StateSyncManager 和 EventSyncManager 合并为统一的 NetworkSyncManager，
//  减少代码重复，简化调用链，提升可维护性。
//
//  ## 设计优势
//
//  1. **统一接口**: 状态和事件通过同一管理器处理
//  2. **减少重复**: 共享冲突解决、序列化等逻辑
//  3. **简化依赖**: 客户端代码只需依赖一个管理器
//  4. **更好的性能**: 统一批处理和优化机会
...
pub struct NetworkSyncConfig {
    ...
    /// 事件确认超时（毫秒）
    pub event_confirmation_timeout_ms: u64,
}
...
impl Default for NetworkSyncConfig {
    fn default() -> Self {
        Self {
            ...
            event_confirmation_timeout_ms: 5000,
        }
    }
}
...
pub struct NetworkSyncManager {
    ...
    /// 事件队列
    event_queue: Vec<SyncNetworkEvent>,
    /// 已确认的事件ID集合
    confirmed_events: HashSet<u64>,
}
...
impl NetworkSyncManager {
    ...
    pub fn new(config: NetworkSyncConfig) -> Self {
        Self {
            ...
            event_queue: Vec::new(),
            confirmed_events: HashSet::new(),
        }
    }

    /// 使用默认配置创建
    pub fn default_config() -> Self {
        Self::new(NetworkSyncConfig::default())
    }
        ...
        if let Some(sync_state) = self.entity_states.get_mut(&entity_id) {
            ...
            if let Some(ref client_state) = client_state_clone {
                let should_check = if let Some(ref _old_server) = old_server_state {
                    !state.is_similar_to(client_state, self.config.conflict_threshold)
                } else {
                    !state.is_similar_to(client_state, self.config.conflict_threshold)
                };
    ...
    /// 生成状态同步数据（增量）
    pub fn generate_state_sync(&mut self, current_tick: u64) -> Result<DeltaPacket, NetworkError> {
        let mut deltas = Vec::new();

        for (entity_id, sync_state) in &self.entity_states {
            // 检查是否需要同步
            if current_tick - sync_state.last_sync_tick < self.config.sync_interval {
                continue;
            }
    ...
    /// 添加网络事件
    pub fn add_event(&mut self, event: SyncNetworkEvent) {
        self.event_queue.push(event);

        // 限制队列大小
        while self.event_queue.len() > self.config.max_event_queue_size {
            self.event_queue.remove(0);
        }
    }

    /// 获取未确认的事件
    pub fn get_unconfirmed_events(&self) -> Vec<&SyncNetworkEvent> {
        let now = current_timestamp_ms();
        self.event_queue
            .iter()
            .filter(|e| {
                !self.confirmed_events.contains(&e.event_id)
                    && now - e.timestamp < self.config.event_confirmation_timeout_ms
            })
            .collect()
    }
    ...
    /// 清理已确认的事件
    pub fn cleanup_confirmed_events(&mut self) {
        let confirmed_ids: Vec<u64> = self.confirmed_events.iter().copied().collect();
        self.event_queue.retain(|e| !confirmed_ids.contains(&e.event_id));
    ...
    /// 获取统计信息
    pub fn get_stats(&self) -> NetworkSyncStats {
        NetworkSyncStats {
            registered_entities: self.entity_states.len(),
            pending_events: self.event_queue.len(),
            confirmed_events: self.confirmed_events.len(),
        }
    }
}
...
mod tests {
    ...
    fn test_network_sync_manager() {
        let mut manager = NetworkSyncManager::default_config();
    ...
    #[test]
    fn test_stats() {
        let manager = NetworkSyncManager::default_config();
        let stats = manager.get_stats();

        assert_eq!(stats.registered_entities, 0);
        assert_eq!(stats.pending_events, 0);
        assert_eq!(stats.confirmed_events, 0);
    }
}
