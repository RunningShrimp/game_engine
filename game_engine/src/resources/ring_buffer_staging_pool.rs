...
//  ```text
//  ┌─────────────────────────────────────────────────────────┐
//  │         Ring Buffer Staging Buffer Pool               │
//  ├─────────────────────────────────────────────────────────┤
//  │  1. 环形缓冲区集成                                    │
...
#[derive(Debug)]
pub struct RingBufferStagingBuffer {
    /// 内存块
...
impl RingBufferStagingBuffer {
    /// 创建新的环形缓冲区Staging Buffer
    pub fn new(
        size: u64,
        allocator: Option<Arc<Mutex<SmartMemoryAllocator>>>,
        preallocation_manager: Option<Arc<Mutex<PreallocationManager>>>,
...
// ============================================================================
// 环形缓冲区Staging Buffer池
// ============================================================================

/// 环形缓冲区Staging Buffer池统计信息
#[derive(Default, Clone, Debug)]
pub struct RingBufferPoolStats {
    /// 基础统计信息
...
#[derive(Debug)]
pub struct RingBufferStagingPool {
    /// 智能内存分配器
    allocator: Arc<Mutex<SmartMemoryAllocator>>,
    /// 预分配管理器
    preallocation_manager: Arc<Mutex<PreallocationManager>>,
    /// 活跃的Staging Buffer
    active_buffers: VecDeque<RingBufferStagingBuffer>,
    /// 空闲的Staging Buffer
    free_buffers: VecDeque<RingBufferStagingBuffer>,
    /// 统计信息
    stats: Arc<Mutex<RingBufferPoolStats>>,
    /// 设备引用
...
impl RingBufferStagingPool {
    /// 创建新的环形缓冲区Staging Buffer池
    pub fn new(device: Arc<wgpu::Device>) -> Self {
        ...
        Self {
            ...
            stats: Arc::new(Mutex::new(RingBufferPoolStats::default())),
            device,
    ...
    pub fn initialize(&mut self) {
        // 预分配管理器会在创建时自动初始化
        tracing::info!(target: "ring_buffer_staging_pool", "Ring Buffer Staging Buffer Pool initialized");
    }
    ...
    pub fn allocate(&mut self, size: u64, alignment: u64) -> (usize, u64) {
        ...
        let mut buffer =
            if let Some(block) = self.preallocation_manager.lock().allocate(request.clone(), 0) {
                let mut enhanced_buffer = RingBufferStagingBuffer::from_block(
                    block,
                ...
                if let Some(result) = self.allocator.lock().allocate(request.clone()) {
                    RingBufferStagingBuffer::from_block(
                        result.block,
                        Some(self.allocator.clone()),
                        Some(self.preallocation_manager.clone()),
                    )
        ...
        // 尝试写入数据（这里简化处理）
        let offset = buffer.write(&[], alignment).unwrap_or_default();
    ...
    /// 创建回退缓冲区
    fn create_fallback_buffer(&self, size: u64) -> RingBufferStagingBuffer {
        // 创建新的GPU缓冲区
    ...
    /// 获取缓冲区引用
    pub fn get_buffer(&self, index: usize) -> Option<&RingBufferStagingBuffer> {
        self.active_buffers.get(index)
    }

    /// 获取可变缓冲区引用
    pub fn get_buffer_mut(&mut self, index: usize) -> Option<&mut RingBufferStagingBuffer> {
    ...
    /// 获取统计信息
    pub fn stats(&self) -> RingBufferPoolStats {
    ...
    pub fn force_gc(&mut self) {
        ...
        tracing::debug!(target: "ring_buffer_staging_pool", "Forced garbage collection completed");
    }
    ...
    /// 获取性能指标
    pub fn performance_metrics(&self) -> RingBufferPerformanceMetrics {
...
impl Default for RingBufferStagingPool {
    fn default() -> Self {
        // 这里需要device，但Default实现不能提供
        // 实际使用时应该通过new()创建
        panic!("RingBufferStagingPool::default() not supported, use new() instead")
    }
}
...
/// 创建环形缓冲区Staging Buffer池
pub fn create_ring_buffer_staging_pool(device: Arc<wgpu::Device>) -> RingBufferStagingPool {
    let mut pool = RingBufferStagingPool::new(device);
    pool.initialize();
    pool
}
...
mod tests {
    ...
    #[test]
    fn test_ring_buffer_staging_buffer_creation() {
        let buffer = RingBufferStagingBuffer::new(1024, None, None);
    ...
    #[test]
    fn test_ring_buffer_staging_buffer_write() {
        let mut buffer = RingBufferStagingBuffer::new(1024, None, None);
    ...
    #[test]
    fn test_ring_buffer_pool_stats() {
        let stats = RingBufferPoolStats::default();
    ...
    fn test_performance_metrics() {
        let metrics = RingBufferPerformanceMetrics {
