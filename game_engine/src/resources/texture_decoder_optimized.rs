...
impl OptimizedTextureDecoder {
    ...
    async fn decode_standard(&self, image_data: Vec<u8>) -> Result<DecodedTexture, TextureDecodeError> {
        spawn_blocking(move || {
            ...
            // 建议格式
            let suggested_format = if self.config.auto_format_optimization {
    ...
    pub async fn decode_batch(
        ...
        for image_data in image_data_list {
            ...
            tasks.push(decoder.decode(image_data));
        }
...
impl Clone for OptimizedTextureDecoder {
    fn clone(&self) -> Self {
        Self {
