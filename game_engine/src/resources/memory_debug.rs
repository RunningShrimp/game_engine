...
use super::memory_monitor::{MemorySnapshot, MemoryMonitor};
    ...
    pub fn mark_freed(&mut self) {
        ...
        if let Ok(allocation_duration) = self.allocation_time.duration_since(std::time::UNIX_EPOCH) {
            if let Ok(free_duration) = self.free_time.unwrap().duration_since(std::time::UNIX_EPOCH) {
                let lifetime = free_duration.as_micros().saturating_sub(allocation_duration.as_micros());
...
impl MemoryDebugger {
    ...
    pub fn update_visualization(&mut self) {
        ...
        // 检查是否到了更新间隔
        if now.duration_since(last_update).as_millis() < self.config.chart_update_interval_ms as u128 {
            return;
        }
    ...
    fn initialize_chart_data(&mut self) {
        ...
        charts.insert(ChartType::MemoryUtilization, PerformanceChartData {
        ...
        charts.insert(ChartType::AllocationLatency, PerformanceChartData {
        ...
        charts.insert(ChartType::AllocationFrequency, PerformanceChartData {
        ...
        charts.insert(ChartType::FragmentationRatio, PerformanceChartData {
        ...
        charts.insert(ChartType::PreallocationHitRate, PerformanceChartData {
    ...
    pub fn tracing_info(&self) -> String {
        ...
        let leaked_allocations = traces.values().filter(|t| {
            !t.is_freed && t.allocation_time.elapsed().unwrap_or(Duration::from_secs(0)).as_secs() > 60
        }).count();
