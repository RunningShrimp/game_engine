...
// 移除了 TypeDowncast trait，因为我们在 DomainEvent 中直接提供了 as_any 方法
...
impl EventBus {
    ...
    /// 发布事件
    pub fn publish<E: DomainEvent>(&self, event: &E) -> Result<(), EventError> {
        let event_type_id = std::any::TypeId::of::<E>();
...
pub trait SnapshotStore: Send + Sync + std::fmt::Debug {
    ...
    /// 获取聚合的所有快照
    fn get_aggregate_snapshots(&self, aggregate_id: u32) -> Vec<Snapshot>;
...
pub struct EventSourcingManager {
    ...
    /// 事件类型注册表
    pub event_registry: Arc<registry::EventTypeRegistry>,
    /// 事件序列号生成器
...
impl DomainEvent for EntityCreatedEvent {
    ...
    fn apply(&self, world: &mut World) -> Result<(), EventError> {
        ...
        // 这里可以根据entity_type创建相应的组件
        // 为了简化，我们只是记录实体创建事件
        tracing::debug!(target: "event_sourcing", "Created entity {} of type {}", self.entity_id, self.entity_type);
        Ok(())
    }
...
impl DomainEvent for EntityDeletedEvent {
    fn event_type(&self) -> &'static str {
        "EntityDeleted"
    }
...
pub struct EntityUpdatedEvent {
    pub entity_id: u32,
    pub entity_type: String,
    pub old_data: Vec<u8>, // 序列化的旧数据
    pub new_data: Vec<u8>, // 序列化的新数据
}

impl DomainEvent for EntityUpdatedEvent {
    fn event_type(&self) -> &'static str {
        "EntityUpdated"
    }
