...
//! ```bash
//! # 将glTF转换为GLB
//! cargo run --bin convert -- input.gltf output.glb
//!
//! # 将GLB转换为OBJ
//! cargo run --bin convert -- model.glb model.obj
//!
//! # 将OBJ转换为glTF
//! cargo run --bin convert -- mesh.obj output.gltf
//! ```
            ...
            for buffer in gltf.buffers() {
                match buffer.source() {
                    ...
                    gltf::buffer::Source::Uri(uri) => {
                        ...
                        let data = fs::read(&uri_path).map_err(|e| ConversionError::Io(e))?;
            ...
            "f" => {
                ...
                for part in &parts[1..] {
                    ...
                    let v_idx = if parts.len() > 0 && !parts[0].is_empty() {
                        parts[0].parse::<usize>().ok().map(|i| if i == 0 { 0 } else { i - 1 }) // OBJ索引从1开始
                    } else {
                        None
                    };
