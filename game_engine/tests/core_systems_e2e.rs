...
mod core_systems_e2e_tests {
    ...
    fn test_all_rigid_body_types() {
        ...
        let dynamic_body = RigidBody::new(dynamic_id, RigidBodyType::Dynamic, Vec3::new(0.0, 5.0, 0.0));
        assert_eq!(dynamic_body.body_type(), RigidBodyType::Dynamic);

        let kinematic_id = RigidBodyId::new(3);
        let kinematic_body = RigidBody::new(kinematic_id, RigidBodyType::Kinematic, Vec3::new(1.0, 0.0, 0.0));
    ...
    fn test_error_conversion_workflow() {
        // 测试错误转换工作流程
        let physics_error = PhysicsError::BodyNotFound("test_body".to_string());
            ...
            DomainError::Physics(err) => {
                match err {
                    PhysicsError::BodyNotFound(id) => {
                        assert_eq!(id, "test_body");
                    }
                    _ => panic!("Expected BodyNotFound error"),
                }
            }
    ...
    fn test_all_error_types() {
        // 测试所有错误类型的可构造性
        let _ = AudioError::SourceNotFound("test".to_string());
        let _ = PhysicsError::BodyNotFound("test".to_string());
