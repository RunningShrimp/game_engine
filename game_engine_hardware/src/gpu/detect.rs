...
fn detect_gpu_wgpu() -> Option<GpuInfo> {
    use wgpu::{Instance, Backends, DeviceType as WgpuDeviceType};
    ...
    let adapter = adapters.iter()
        .find(|a| a.get_info().device_type == WgpuDeviceType::DiscreteGpu)
        .or_else(|| adapters.iter()
            .find(|a| a.get_info().device_type == WgpuDeviceType::IntegratedGpu))
        ...
        if let Ok(text) = String::from_utf8(output.stdout) {
            for line in text.lines() {
                if line.contains("Chipset Model:") {
                    ...
                    let vendor = detect_vendor_from_name(&name);
                    let tier = classify_gpu_tier(vendor, &name, DeviceType::IntegratedGpu);
...
/// 分类GPU性能等级
fn classify_gpu_tier(vendor: GpuVendor, name: &str, device_type: DeviceType) -> GpuTier {
    let name_lower = name.to_lowercase();
...
fn estimate_vram(name: &str, device_type: DeviceType) -> u64 {
    ...
    if name_lower.contains("4090") {
        ...
        6144
    } else if device_type == DeviceType::DiscreteGpu {
        4096
    } else {
        2048 // 集成显卡
    }
...
mod tests {
    ...
    fn test_gpu_detection() {
        let gpu = detect_gpu();
        tracing::info!(target: "hardware", "Detected GPU: {:#?}", gpu);
