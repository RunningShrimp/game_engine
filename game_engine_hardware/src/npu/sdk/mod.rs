...
use crate::npu::detect::{NpuInfo, NpuVendor};
use std::path::Path;
...
impl NpuSdkManager {
    ...
    pub fn create_engine(
        &self,
        backend: Option<NpuBackend>,
    ) -> HardwareResult<Box<dyn NpuInferenceEngine>> {
        let backend = backend.or(self.preferred_backend).ok_or_else(|| {
        ...
        match backend {
            ...
            NpuBackend::TensorRT => Ok(Box::new(TensorRTEngine::new()?)),
            NpuBackend::OnnxRuntime => Ok(Box::new(OnnxRuntimeEngine::new()?)),
            NpuBackend::OpenVINO => {
                use crate::npu::sdk::extended::OpenVINOEngine;
                Ok(Box::new(OpenVINOEngine::new()?))
            }
            NpuBackend::ROCm => {
                use crate::npu::sdk::extended::ROCmEngine;
                Ok(Box::new(ROCmEngine::new()?))
            }
            NpuBackend::Ascend => {
                use crate::npu::sdk::extended::AscendEngine;
                Ok(Box::new(AscendEngine::new()?))
            }
            NpuBackend::SNPE => {
                use crate::npu::sdk::extended::SNPEEngine;
            ...
            NpuBackend::CpuFallback => Ok(Box::new(CpuFallbackEngine::new())),
        }
    }
