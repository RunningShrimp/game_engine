...
impl NpuAccelerator {
    ...
    pub fn new(npu_info: Option<NpuInfo>) -> Self {
        let enabled =
            npu_info.is_some() && npu_info.as_ref().map(|n| n.tops > 5.0).unwrap_or(false);
    ...
    pub fn predict_physics_batch(
        ...
        objects
            .iter()
            .filter_map(|(id, pos, vel)| self.predict_physics(*id, *pos, *vel, time_delta))
            .collect()
    }
    ...
    ///
    /// 使用NPU生成地形、纹理等程序化内容
    pub fn generate_procedural_terrain(&self, seed: u64, size: (u32, u32)) -> Option<Vec<f32>> {
        if !self.enabled {
            return None;
        }
