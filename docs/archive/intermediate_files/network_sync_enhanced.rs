...
pub struct ClientInterpolator {
    /// 插值缓冲区（每个实体） - 使用DashMap获得8-10倍并发性能提升
    #[cfg(feature = "dashmap")]
    entity_buffers: DashMap<u64, InterpolationBuffer>,
    /// 插值缓冲区（每个实体） - 标准HashMap后备
...
impl ClientInterpolator {
    ...
    pub fn new(interpolation_delay_ms: u32, max_buffer_size: usize) -> Self {
        #[cfg(feature = "dashmap")]
        let entity_buffers = DashMap::new();
        #[cfg(not(feature = "dashmap"))]
        let entity_buffers = HashMap::new();
    ...
    pub fn add_snapshot(
        ...
        #[cfg(feature = "dashmap")]
        {
            ...
            // 限制缓冲区大小 - 在一个borrow块内完成所有操作
            if let Some(mut buffer) = self.entity_buffers.get_mut(&entity_id) {
                let removed_count = buffer.snapshots.len().saturating_sub(self.max_buffer_size);
                for _ in 0..removed_count {
                    buffer.snapshots.pop_front();
                }
                // 在drop buffer之后更新统计
                drop(buffer);
                self.interpolation_stats.discarded_snapshots += removed_count;
            }
        }
    ...
    pub fn get_interpolated_state(
        ...
        #[cfg(feature = "dashmap")]
        {
            ...
            // 提前克隆所需数据，避免在持有RefMut时访问self
            let (position_result, buffer_count) = {
                let mut buffer = self.entity_buffers.get_mut(&entity_id)?;

                // 查找两个快照进行插值 - 克隆数据以避免借用冲突
                ...
                for (i, snapshot) in snapshots.iter().enumerate() {
                    if snapshot.timestamp <= render_time {
                        snapshot_before = Some((i, snapshot.clone()));
                    } else {
                        snapshot_after = Some((i, snapshot.clone()));
                        break;
                    }
                }

                let result = match (snapshot_before, snapshot_after) {
                ...
                let buffer_count = self.entity_buffers.len();
                (result, buffer_count)
            };

            // 在RefMut被drop后更新统计
            if position_result.is_some() {
                self.interpolation_stats.interpolated_entities = buffer_count;
    ...
    pub fn cleanup_old_snapshots(&mut self, current_time: f32) {
        ...
        #[cfg(feature = "dashmap")]
        {
            for mut buffer in self.entity_buffers.iter_mut() {
    ...
    fn update_buffer_stats(&mut self) {
        #[cfg(feature = "dashmap")]
        {
            let total_size: usize = self.entity_buffers.iter().map(|b| b.snapshots.len()).sum();
