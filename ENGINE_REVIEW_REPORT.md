...
**使用场景**:
- 平台适配（target_arch = "wasm32"、target_os等）
- 特性开关（feature = "gltf"、feature = "xr"等）
- 测试代码（cfg(test)）
...
### 12.2 已完成的改进（2025-12-25）

1. **代码清理**:
   - ✅ 删除遗留监控模块（monitoring_legacy.rs），所有功能已迁移到system_monitor
   - ✅ 删除重复的渲染优化实现（render_optimization.rs），使用render模块中的完整实现
   - ✅ 标记Script组件为deprecated，引导使用ScriptComponent

2. **模块整合**:
   - ✅ pipeline_optimization.rs已在render模块中正确组织
   - ✅ gpu/optimization.rs已在gpu模块中正确组织
...
3. **长期改进**（3-6月）:
   - 添加移动平台支持
   - 实现更多后处理效果（SSR、体积光、程序化噪声）
   - 支持更多音频效果（音频遮挡、混响、压缩/限制器）
