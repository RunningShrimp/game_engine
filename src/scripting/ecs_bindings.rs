    ...
    pub fn register_api(&self, api: &mut ScriptApi) {
            ...
            if let Some(ScriptValue::Int(entity_id)) = args.first() {
                ...
                if let Some(transform) = world.get::<Transform>(entity) {
                    let value = ExtendedScriptValue::Object(
                        vec![
                            ("position".to_string(), ExtendedScriptValue::Vec3(transform.pos)),
                            ("rotation".to_string(), ExtendedScriptValue::Quat(transform.rot)),
                            ("scale".to_string(), ExtendedScriptValue::Vec3(transform.scale)),
                        ].into_iter().collect()
                    );
        ...
        api.register_function("set_rotation", move |args| {
            if let (Some(ScriptValue::Int(entity_id)), Some(ScriptValue::Float(x)), Some(ScriptValue::Float(y)), Some(ScriptValue::Float(z))) = 
                ...
                ScriptResult::Error("set_rotation() requires entity_id, x, y, z (degrees)".to_string())
            }
        });
...
mod tests {
    ...
    fn test_ecs_bindings() {
        ...
        if let ScriptResult::Success(entity_id_str) = result {
            ...
            // 测试设置位置
            let result = api.call("set_position", &[
