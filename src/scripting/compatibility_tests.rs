...
mod tests {
    use crate::scripting::system::{
        ScriptContext, ScriptSystem, ScriptLanguage, ScriptValue, ScriptResult
    };
    ...
    fn test_lua_specific_features(context: &mut LuaContext) {
        ...
        let result = context.call_function(
            "add",
            &[ScriptValue::Int(3), ScriptValue::Int(4)]
        );
        
        assert!(
            matches!(result, ScriptResult::Success(ref s) if s == "7"),
            "Lua function call should return '7', got {:?}",
            result
        );
    ...
    fn test_script_system_multilanguage_support() {
        ...
        // 测试全局变量在不同语言间的隔离
        system.set_global(ScriptLanguage::Lua, "test", ScriptValue::String("Lua".to_string()));
        system.set_global(ScriptLanguage::JavaScript, "test", ScriptValue::String("JS".to_string()));
    ...
    fn test_script_value_type_compatibility() {
        let test_values = vec![
            ...
            ScriptValue::Object(std::collections::HashMap::from([
                ("key".to_string(), ScriptValue::String("value".to_string()))
            ])),
        ];
        ...
        for (i, value) in test_values.iter().enumerate() {
            ...
            let lua_set = lua_context.set_global(&var_name, value.clone());
            ...
            match value {
                ScriptValue::Null => {
                    ...
                    assert!(
                        matches!(lua_set, ScriptResult::Success(_) | ScriptResult::Void),
                        "Lua should handle Null value type"
                    );
                    assert!(
                ...
                ScriptValue::Bool(_) | ScriptValue::Int(_) |
                ScriptValue::Float(_) | ScriptValue::String(_) => {
                    assert!(
                        matches!(lua_set, ScriptResult::Success(_)),
                        "Lua should handle basic value type {:?}",
                        value
                    );
                    assert!(
                        matches!(js_set, ScriptResult::Success(_) | ScriptResult::Void),
                ...
                ScriptValue::Array(_) | ScriptValue::Object(_) => {
                    ...
                    // 对于复杂类型，我们期望至少不失败
                    assert!(
                        matches!(lua_set, ScriptResult::Success(_) | ScriptResult::Void | ScriptResult::Error(_)),
                        "Lua should handle complex value type without panicking"
                    );
                    assert!(
                        matches!(js_set, ScriptResult::Success(_) | ScriptResult::Void | ScriptResult::Error(_)),
    ...
    fn test_error_handling_consistency() {
        ...
        assert!(
            matches!(lua_syntax_error, ScriptResult::Error(_)),
        ...
        // 测试函数不存在错误
        let lua_func_error = lua_context.call_function("nonexistent_function", &[]);
        let js_func_error = js_context.call_function("nonexistent_function", &[]);

        assert!(
            matches!(lua_func_error, ScriptResult::Error(_)),
