    ...
    fn register_physics_api(&self, api: &mut ScriptApi) {
        ...
        api.register_function("set_angular_velocity", move |args| {
            ...
            ) = (args.first(), args.get(1), args.get(2), args.get(3))
            {
                ...
                ScriptResult::Error(
                    "set_angular_velocity() requires entity_id, x, y, z".to_string(),
                )
            }
        });
        ...
        api.register_function("raycast", move |args| {
            if let (
                ...
                ScriptResult::Error(
                    "raycast() requires origin (ox, oy, oz) and direction (dx, dy, dz)".to_string(),
                )
            }
        });
    }
...
mod tests {
    ...
    fn test_audio_bindings() {
        ...
        let result = api.call(
            "play_sound",
            &[ScriptValue::String("explosion.wav".to_string())],
        );
        assert!(matches!(result, ScriptResult::Success(_)));
