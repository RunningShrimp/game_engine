...
pub enum CommandError {
    ...
    UndoFailed(String),
    /// 无法合并
    #[error("Commands cannot be merged")]
    CannotMerge,
    /// 无效状态
...
impl CommandManager {
    ...
    pub fn execute(
        ...
        if self.is_undoing {
            return Err(CommandError::InvalidState(
                "Cannot execute while undoing".into(),
            ));
        }
    ...
    pub fn redo(&mut self, context: &mut dyn Any) -> Result<bool, CommandError> {
        if self.is_undoing {
            return Err(CommandError::InvalidState(
                "Cannot redo while undoing".into(),
            ));
        }
...
mod tests {
    ...
    fn test_composite_command() {
        ...
        composite.add(Box::new(TestCommand {
            value: 1,
            executed: false,
        }));
        composite.add(Box::new(TestCommand {
