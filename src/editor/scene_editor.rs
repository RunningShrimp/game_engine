...
impl SceneEditor {
    ...
    pub fn render(&mut self, ui: &mut egui::Ui, world: &mut World) {
        ...
        ui.collapsing("Camera", |ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("Rotation:");
                ui.add(egui::DragValue::new(&mut self.camera_rotation.x).prefix("X: ").speed(1.0).suffix("°"));
                ui.add(egui::DragValue::new(&mut self.camera_rotation.y).prefix("Y: ").speed(1.0).suffix("°"));
                ui.add(egui::DragValue::new(&mut self.camera_rotation.z).prefix("Z: ").speed(1.0).suffix("°"));
            });
    ...
    fn draw_grid(&self, painter: &egui::Painter, rect: egui::Rect) {
        ...
        // X轴 (红色)
        painter.line_segment(
            [egui::pos2(center_x, center_y), egui::pos2(center_x + 50.0, center_y)],
            egui::Stroke::new(2.0, egui::Color32::from_rgb(255, 0, 0)),
        );
        
        // Y轴 (绿色)
        painter.line_segment(
            [egui::pos2(center_x, center_y), egui::pos2(center_x, center_y - 50.0)],
    ...
    /// 世界坐标转屏幕坐标
    fn world_to_screen(&self, world_pos: Vec3, rect: egui::Rect) -> egui::Pos2 {
    ...
    fn handle_click(&mut self, click_pos: egui::Pos2, rect: egui::Rect, world: &mut World) {
        ...
        for (entity, transform) in query.iter(world) {
            let screen_pos = self.world_to_screen(transform.pos, rect);
            let distance = ((click_pos.x - screen_pos.x).powi(2) + (click_pos.y - screen_pos.y).powi(2)).sqrt();
