...
use super::{Input, InputEvent, KeyCode, MouseButton, Modifiers};
use wasm_bindgen::prelude::*;
use wasm_bindgen::JsCast;
use web_sys::{Window, HtmlCanvasElement, KeyboardEvent, MouseEvent, WheelEvent, TouchEvent};
    ...
    fn setup_event_listeners(&mut self) -> Result<(), JsValue> {
        // 键盘事件
        {
            ...
            let closure = Closure::wrap(Box::new(move |event: KeyboardEvent| {
                if let Some(key_code) = map_key_code(&event.code()) {
                    ...
                    events.lock().unwrap().push(InputEvent::KeyPressed { key: key_code, modifiers });
                }
            }) as Box<dyn FnMut(_)>);
            self.window.add_event_listener_with_callback("keydown", closure.as_ref().unchecked_ref())?;
            closure.forget();
        }
        
        {
            ...
            let closure = Closure::wrap(Box::new(move |event: KeyboardEvent| {
                if let Some(key_code) = map_key_code(&event.code()) {
                    ...
                    events.lock().unwrap().push(InputEvent::KeyReleased { key: key_code, modifiers });
                }
            }) as Box<dyn FnMut(_)>);
            self.window.add_event_listener_with_callback("keyup", closure.as_ref().unchecked_ref())?;
            closure.forget();
        }
        ...
        
        {
            ...
            let closure = Closure::wrap(Box::new(move |event: MouseEvent| {
                ...
                events.lock().unwrap().push(InputEvent::MouseButtonPressed { button, x, y });
            }) as Box<dyn FnMut(_)>);
        ...
        
        {
            ...
            let closure = Closure::wrap(Box::new(move |event: MouseEvent| {
                ...
                events.lock().unwrap().push(InputEvent::MouseButtonReleased { button, x, y });
            }) as Box<dyn FnMut(_)>);
        ...
        // 滚轮事件
        {
            ...
            let closure = Closure::wrap(Box::new(move |event: WheelEvent| {
                ...
                events.lock().unwrap().push(InputEvent::MouseWheel { delta_x, delta_y });
            }) as Box<dyn FnMut(_)>);
...
fn map_key_code(code: &str) -> Option<KeyCode> {
    Some(match code {
        ...
        "Digit0" => KeyCode::Num0, "Digit1" => KeyCode::Num1, "Digit2" => KeyCode::Num2,
        "Digit3" => KeyCode::Num3, "Digit4" => KeyCode::Num4, "Digit5" => KeyCode::Num5,
        "Digit6" => KeyCode::Num6, "Digit7" => KeyCode::Num7, "Digit8" => KeyCode::Num8,
        "Digit9" => KeyCode::Num9,
        ...
        "Delete" => KeyCode::Delete, "ArrowLeft" => KeyCode::Left, "ArrowRight" => KeyCode::Right,
        "ArrowUp" => KeyCode::Up, "ArrowDown" => KeyCode::Down, "Home" => KeyCode::Home,
