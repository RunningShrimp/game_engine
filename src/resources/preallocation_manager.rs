...
use super::memory_allocator::{AllocationRequest, AllocationType, AllocationPriority};
...
impl AllocationPattern {
    ...
    fn update_statistics(&mut self) {
        let total_allocs = self.small_allocations + self.medium_allocations + self.large_allocations;
        if total_allocs > 0 {
    ...
    pub fn predict_next_frame需求(&self) -> HashMap<BlockSize, u32> {
        ...
        if self.small_allocations > 0 {
            prediction.insert(BlockSize::Small, (self.small_allocations / PREALLOCATION_HISTORY_LENGTH as u64) as u32);
        }
        if self.medium_allocations > 0 {
            prediction.insert(BlockSize::Medium, (self.medium_allocations / PREALLOCATION_HISTORY_LENGTH as u64) as u32);
        }
        if self.large_allocations > 0 {
            prediction.insert(BlockSize::Large, (self.large_allocations / PREALLOCATION_HISTORY_LENGTH as u64) as u32);
        }
...
impl PreallocatedBlock {
    ...
    pub fn can_reclaim(&self, current_frame: u64, reclaim_delay: u64) -> bool {
        ...
        if let Some(last_used) = self.last_used {
            ...
            frames_since_use >= reclaim_delay
        } else {
            // 从未使用过的块可以立即回收
            true
        }
...
impl PreallocationManager {
    ...
    /// 分配预分配块
    pub fn allocate(&mut self, request: AllocationRequest) -> Option<MemoryBlock> {
    ...
    fn reclaim_unused_blocks(&mut self) {
        ...
        for (block_size, pool) in &mut self.preallocated_pools {
            ...
            for &i in to_reclaim.iter().rev() {
                if let Some(mut block) = pool.remove(i) {
                    // 释放到环形池
                    ...
                    // 更新统计信息
                    {
                        ...
                        stats.current_preallocated_blocks = stats.current_preallocated_blocks.saturating_sub(1);
                    }
                }
            }
        }
...
/// 创建高性能配置的预分配管理器
pub fn create_high_performance_preallocation_manager(device: Arc<wgpu::Device>) -> PreallocationManager {
...
mod tests {
    ...
    fn test_preallocation_config_default() {
        ...
        assert_eq!(config.initial_preallocation_size, DEFAULT_PREALLOCATION_SIZE);
