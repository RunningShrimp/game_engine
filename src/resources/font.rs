...
#[derive(Debug, Deserialize)]
pub struct MsdfDoc { pub common: Common, pub info: Option<Info>, pub chars: Vec<CharEntry> }
...
impl MsdfFontAtlas {
    ...
    pub fn glyph(&self, codepoint: u32) -> Option<([f32;2],[f32;2],[f32;2],[f32;2], f32, f32)> {
        let g = self.glyphs.get(&codepoint)?;
        Some((g.uv_offset, g.uv_scale, g.size, g.bearing, g.advance, g.px_range))
    }
}
