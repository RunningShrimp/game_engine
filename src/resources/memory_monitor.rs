...
use super::enhanced_staging_buffer::{EnhancedStagingBufferPool, EnhancedPerformanceMetrics};
use super::memory_allocator::{MemoryPressure, MemoryPressureEvent};
use super::ring_buffer_pool::AllocationStats;
...
impl MemorySnapshot {
    ...
    pub fn from_metrics(metrics: &EnhancedPerformanceMetrics, total_capacity: u64) -> Self {
        Self {
            ...
            free_bytes: total_capacity - (total_capacity as f32 * metrics.current_utilization) as u64,
...
impl PerformanceAnalysis {
    ...
    pub fn calculate_efficiency_score(&mut self) {
        ...
        // 加权平均
        self.efficiency_score = (utilization_score * 0.4 + latency_score * 0.4 + hit_rate_score * 0.2).min(1.0);
    }
...
impl LeakDetectionResult {
    ...
    pub fn generate_fix_recommendations(&mut self) {
        ...
        if self.has_leak {
            ...
            self.recommendations.push(format!("泄漏量: {:.1}MB, 泄漏块数: {}", 
...
impl MemoryMonitor {
    ...
    fn collect_memory_snapshot(&self) -> MemorySnapshot {
        ...
        MemorySnapshot {
            ...
            utilization: if total_capacity > 0 { total_used as f32 / total_capacity as f32 } else { 0.0 },
    ...
    /// 生成压力建议
    fn generate_pressure_recommendation(&self, pressure: &MemoryPressure) -> super::memory_allocator::PressureRecommendation {
    ...
    fn detect_memory_leaks(&self, snapshot: &MemorySnapshot) {
        ...
        let allocation_increase = snapshot.allocation_count.saturating_sub(recent_snapshots.first().unwrap().allocation_count);
        let deallocation_increase = snapshot.deallocation_count.saturating_sub(recent_snapshots.first().unwrap().deallocation_count);
    ...
    fn perform_performance_analysis(&mut self) {
        ...
        analysis.average_allocation_latency = latencies.iter().sum::<f32>() / latencies.len() as f32;
