...
pub mod runtime;
pub use runtime::{block_on, global_runtime, spawn};
...
impl AssetLoader {
    ...
    pub async fn load_bytes(&self, path: &Path) -> Result<Vec<u8>, ()> {
        ...
        async fn idb_get_bytes(url: &str) -> Result<Option<Vec<u8>>, ()> {
            ...
            let (tx, rx) = futures::channel::oneshot::channel();
            {
                ...
                let closure =
                    wasm_bindgen::closure::Closure::wrap(Box::new(move |e: web_sys::Event| {
                        if let Ok(open_req) =
                            e.target().unwrap().dyn_into::<web_sys::IdbOpenDbRequest>()
                        {
                            if let Ok(db) = open_req
                                .result()
                                .and_then(|x| x.dyn_into::<web_sys::IdbDatabase>())
                            {
                                let txn = db
                                ...
                                let get_req = store
                                    .get(&wasm_bindgen::JsValue::from_str(&url_string))
                                    .unwrap();
                                let inner_tx = tx;
                                let success = wasm_bindgen::closure::Closure::wrap(Box::new(
                                    move |ev: web_sys::Event| {
                                        ...
                                        if result.is_undefined() || result.is_null() {
                                            ...
                                            let bytes_val = js_sys::Reflect::get(
                                                &result,
        ...
        if let Some(bytes) = idb_get_bytes(&path.to_string_lossy()).await? {
            return Ok(bytes);
        }
...
pub use coroutine_loader::{
    AssetType, CoroutineAssetLoader, CoroutineLoaderConfig, LoadComplete, LoadError, LoadHandle,
    LoadPriority, LoadResult, LoaderStats,
};
