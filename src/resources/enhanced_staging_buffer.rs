...
use super::ring_buffer_pool::{RingBufferPool, MemoryBlock, BlockSize, align_to};
...
impl EnhancedStagingBufferPool {
    ...
    pub fn allocate(&mut self, size: u64, alignment: u64) -> (usize, u64) {
        ...
        // 尝试从预分配管理器分配
        let mut buffer = if let Some(block) = self.preallocation_manager.lock().allocate(request.clone()) {
