...
use super::npu_sdk::{NpuInferenceEngine, NpuBackend};
use super::gpu_detect::{detect_gpu, GpuVendor};
use super::npu_detect::{detect_npu, NpuVendor};
use super::soc_detect::{detect_soc, SocVendor};
use std::path::Path;
...
impl SdkManager {
    ...
    pub fn create_engine_with_backend(
        ...
        match backend {
            ...
            NpuBackend::OpenVINO => {
                use super::openvino_integration::OpenVinoEngine;
                Ok(Box::new(OpenVinoEngine::new()?))
            }
            NpuBackend::ROCm => {
                use super::rocm_integration::RocmEngine;
                Ok(Box::new(RocmEngine::new()?))
            }
            NpuBackend::Ascend => {
                use super::cann_integration::CannEngine;
                Ok(Box::new(CannEngine::new()?))
            }
            NpuBackend::SNPE => {
                use super::snpe_integration::SnpeEngine;
                Ok(Box::new(SnpeEngine::new()?))
            }
            NpuBackend::NeuroPilot => {
                use super::neuropilot_integration::NeuroPilotEngine;
                Ok(Box::new(NeuroPilotEngine::new()?))
            }
            NpuBackend::CoreML => {
                use super::coreml_integration::CoreMlEngine;
...
/// 
/// ```rust,ignore
/// use game_engine::performance::hardware::sdk_manager::auto_create_engine;
/// 
...
/// 
/// ```rust,ignore
/// use game_engine::performance::hardware::sdk_manager::load_model;
/// 
