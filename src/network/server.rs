    ...
    pub fn start_sync(&mut self) -> Result<(), NetworkError> {
        ...
        *crate::error::safe_lock(&self.running, "server_running").unwrap() = true;
        ...
        std::thread::spawn(move || {
            Self::accept_connections_sync(listener, clients, running, config, delay_compensation);
        });
        ...
        std::thread::spawn(move || {
            Self::heartbeat_checker_sync(clients_clone, running_clone, timeout);
        });
