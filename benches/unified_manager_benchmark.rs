//! UnifiedResourceManager 并发性能基准测试
//!
...
#[cfg(not(feature = "dashmap"))]
type CacheMap<K, V> = RwLock<std::collections::HashMap<K, V>>;
...
struct RwLockResourceCache {
    cache: RwLock<std::collections::HashMap<PathBuf, Arc<MockResource>>>,
}
...
fn main() {
    println!("===================================================================");
    println!("UnifiedResourceManager 并发性能基准测试");
    println!("===================================================================");
    println!();
    ...
    #[cfg(feature = "dashmap")]
    {
        ...
        let handles: Vec<_> = (0..NUM_THREADS)
            .map(|thread_id| {
                ...
                std::thread::spawn(move || {
                    for i in 0..NUM_OPERATIONS {
                        let path = PathBuf::from(format!("thread_{}_resource_{}.dat", thread_id, i));
                        let resource = Arc::new(MockResource::new((thread_id * NUM_OPERATIONS + i) as u32));
                        cache.insert(path, resource);
                    }
                })
            })
            ...
            .map(|thread_id| {
                ...
                std::thread::spawn(move || {
                    for i in 0..NUM_OPERATIONS {
                        if i % 10 < 7 {
                            // 70% 读取
                            let path = PathBuf::from(format!("resource_{}.dat", i % (NUM_RESOURCES / 2)));
                            let _ = cache.get(&path);
                        } else {
                            // 30% 写入
                            let path = PathBuf::from(format!("thread_{}_resource_{}.dat", thread_id, i));
