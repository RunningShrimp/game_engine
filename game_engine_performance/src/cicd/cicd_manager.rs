...
impl CicdPipeline {
    ...
    pub fn get_status(&self) -> PipelineStatus {
        if let Some(_failed) = self.stages.iter().find(|s| s.status == StageStatus::Failed) {
...
impl CicdManager {
    ...
    pub fn create_pipeline(
        ...
        let pipeline = CicdPipeline::new(commit_hash, branch);
...
mod tests {
    ...
    fn test_cicd_manager() {
        ...
        assert!(manager.get_pipeline(&id2).is_some());
        assert!(manager.get_latest_pipeline_for_branch("main").is_some());
        assert!(manager.get_latest_pipeline_for_branch("dev").is_some());
    }
