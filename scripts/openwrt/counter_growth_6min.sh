...

{
  ...
  while [ $i -le 3 ]; do
    ...
    $SSH_BASE "$TARGET" '
      ...
      echo "cake_loaded=$(lsmod 2>/dev/null | grep -c "^sch_cake" || true)"
      echo "cake_installed=$(opkg list-installed 2>/dev/null | grep -c "^kmod-sched-cake " || true)"
      echo "qdisc_pppoe_wan=$(tc qdisc show dev pppoe-wan 2>/dev/null | head -n 1 || true)"
